...
do
  grep "all done"  /lustre/f1/unswept/Oar.Esrl.Nggps_psd/scripts/${stream}stream/${stream}stream.out | awk '{print $1,$5,$6,$9}' > ${stream}stream_time
  date2=`date -I`
  date1=$(date -I -d "$d -$ndates day")
  d=$date1
  rm ${stream}_out
  while [ "$d" != "$date2" ]; do 
    grep "`date -d $d +"%b %-d"` " ${stream}stream_time | wc -l >> ${stream}_out
    d=$(date -I -d "$d + 1 day")
  done
